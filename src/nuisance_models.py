...
class PropensityModel(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 记录每个 step 的 Loss
        wandb.log({"train_prop_loss_step": loss.item()})
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # 记录每个 step 的 Loss
        wandb.log({"val_prop_loss_step": loss.item()})
...
class ConditionalQuantileModel(L.LightningModule):
    def __init__(self, quantile: float, input_dim: int, n_treatments: int, lr: float = 1e-3, log_key_suffix: str = ""):
        super(ConditionalQuantileModel, self).__init__()
        self.quantile = quantile
        self.log_key_suffix = log_key_suffix
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 记录每个 step 的 Loss
        wandb.log({f"train_quantile_loss_step{self.log_key_suffix}": loss.item()})
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # 记录每个 step 的 Loss
        wandb.log({f"val_quantile_loss_step{self.log_key_suffix}": loss.item()})

        self.log(f"val_quantile{self.log_key_suffix}_loss", loss, on_step=False, on_epoch=True, prog_bar=True, logger=True)
...
class OutcomeModel(L.LightningModule):
    def __init__(self, input_dim: int, num_treatments: int, type: str, quantile: float, quantile_model: ConditionalQuantileModel = None, lr: float = 1e-3, is_for_alpha_minus: bool = False):
        ...
        self.input_dim = input_dim
        self.num_treatments = num_treatments
        self.is_for_alpha_minus = is_for_alpha_minus
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 动态生成 log_key
        log_key = f"train_{self.type}_{'minus' if self.is_for_alpha_minus else 'plus'}_loss"
        wandb.log({log_key: loss.item()})

        self.log(log_key, loss, on_step=False, on_epoch=True, prog_bar=True, logger=True)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # 动态生成 log_key
        log_key = f"val_{self.type}_{'minus' if self.is_for_alpha_minus else 'plus'}_loss"
        wandb.log({log_key: loss.item()})
