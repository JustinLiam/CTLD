...
class _BaseBLearner:
    ...
    def __init__(self,
        ...
        self.outcome_upper_model_0 = clone(cate_bounds_model, safe=False)
        self.outcome_upper_model_1 = clone(cate_bounds_model, safe=False)
        self.outcome_lower_model_0 = clone(cate_bounds_model, safe=False)
        self.outcome_lower_model_1 = clone(cate_bounds_model, safe=False)
    ...
    def fit(self, X, A, Y, X_val=None, A_val=None, Y_val=None):
        ...
        self._fit_outcomes_with_nuisances(X, A, Y, nuisances, oracle=False)
        return self
    ...
    def outcome_bounds(self, X):
        return (
            self.outcome_lower_model_0.predict(X).flatten(),
            self.outcome_upper_model_0.predict(X).flatten(),
            self.outcome_lower_model_1.predict(X).flatten(),
            self.outcome_upper_model_1.predict(X).flatten()
            )
    ...
    def _fit_outcomes_with_nuisances(self, X, A, Y, nuisances, oracle=False):
        phi_plus_1 = self._get_pseudo_outcome_plus_1(X, A, Y, nuisances)
        phi_minus_0 = self._get_pseudo_outcome_minus_0(X, A, Y, nuisances)
        ...
        #     self.cate_lower_model_oracle.fit(X[:, self.proj_idx] if self.proj_idx is not None else X, phi_minus)
        # else:
        self.outcome_upper_model_0.fit(X[:, self.proj_idx] if self.proj_idx is not None else X, phi_plus_0)
        self.outcome_upper_model_1.fit(X[:, self.proj_idx] if self.proj_idx is not None else X, phi_plus_1)
        self.outcome_lower_model_0.fit(X[:, self.proj_idx] if self.proj_idx is not None else X, phi_minus_0)
        self.outcome_lower_model_1.fit(X[:, self.proj_idx] if self.proj_idx is not None else X, phi_minus_1)
...
class NNBLearner(_BaseBLearner):
    def __init__(
        ...
        if cate_bounds_model is None:
            ...
            cate_bounds_model = MuNet(**final_nn_args)
        super().__init__(
                 nuisance_model=nuisance_model,
                 cate_bounds_model=cate_bounds_model,
                 use_rho=use_rho,
